...
附录 O 图 O.1 的消息 1/2/5 描述 SIP 服务器与独立下载客户端之间的控制腿：消息 1 不携带 SDP，客户端在消息 2 中给出接收端 SDP，消息 5 的 ACK 再携带媒体发送方 SDP。Owl 采用内置 TCP 下载客户端后，这条内部控制腿被进程内调用替代，不再建立一组自呼 SIP 对话。Owl 发给下级设备/NVR 的请求对应规范消息 3，必须携带由平台生成的 `s=Download` SDP；设备 200 OK 对应消息 4。上级平台向 Owl 级联下载时，其带接收端 SDP 的 INVITE 同样位于消息 3 语义，Owl 作为 B2BUA 再建立下级发送方对话并中继文件字节。无 SDP 的消息 1 不能直接送入媒体发送方/级联入口，否则无法确定下载时间段、接收端点和 SSRC。
