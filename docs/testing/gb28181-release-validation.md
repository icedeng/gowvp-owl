...
- 上下级平台级联：基于 UDP/TCP/TLS 的多上级注册、核心及版本化扩展查询、目录、订阅通知、直播、回放、下载和语音广播/对讲已完成自动化验证；TCP/TLS 持久连接覆盖 Digest 注册、心跳、重连事务换连接及 301/302 传输切换，TLS 校验证书并支持客户端证书；2022 附录 H 指定路径已覆盖实时、回放和下载；仍需四版本真实上级平台互通；
