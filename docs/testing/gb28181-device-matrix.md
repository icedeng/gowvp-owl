...
- 对 Keepalive、Alarm、查询应答及控制各重发一次完全相同的 SIP 事务，确认 Owl 返回与首次完全相同的响应且业务状态、回调和向上转发只执行一次；使用抓包回放至少补测一次延迟 20～32 秒的 UDP 原事务重传；保持 `Call-ID/CSeq` 不变但更换 Via branch 时应按新事务处理；
- Keepalive、离线判定和恢复；
- Catalog、目录更新和通道上下线；
