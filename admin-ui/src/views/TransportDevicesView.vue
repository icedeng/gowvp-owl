...
const transportRows = computed(() =>
  rows.value.filter((device) =>
    /JT\/?T?\s*(808|1078)|JT808|JT1078|TRANSPORT/i.test(String(device.type || ""))
  )
);
  ...
  <main class="page-content device-page">
    ...
    <section class="card table-card">
      <div class="toolbar">
        ...
        <span class="toolbar-spacer" /><span class="section-note" aria-live="polite">当前显示 {{ filtered.length }} 台</span>
      </div>
      ...
      <div v-else-if="!filtered.length" class="empty-state empty-action transport-empty">
        ...
        <span>{{ transportRows.length ? "清空搜索条件后可恢复全部设备。" : "现有设备模型仅实现 GB28181 与 ONVIF；JT/T 808、JT/T 1078 接口开放后，设备会自动显示在此页面。" }}</span>
      ...
      <div v-else class="table-wrap">
        <table class="data-table device-data-table">
          ...
          <tbody><tr v-for="device in filtered" :key="device.id"><td>{{ device.name || "未命名设备" }}</td><td class="mono">{{ device.device_id || device.id }}</td><td class="mono">{{ device.address || [device.ip, device.port].filter(Boolean).join(":") || "—" }}</td><td>{{ device.type }}</td><td>{{ device.channels || 0 }} 路</td><td><span class="status" :class="device.is_online ? 'online' : 'offline'">{{ device.is_online ? "在线" : "离线" }}</span></td><td><RouterLink class="btn btn-sm" :to="`/transport-devices/${encodeURIComponent(device.id)}`">详情</RouterLink></td></tr></tbody>
        </table>
      </div>
    </section>
  </main>
